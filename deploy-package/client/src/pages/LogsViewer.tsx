...
const LogsViewer: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedServerId) {
      ...
      const fetchHistory = async () => {
        try {
          const res = await api.get(`/logs/${selectedServerId}?limit=200`);
          if (res.data && Array.isArray(res.data)) {
            // Reverse to show oldest first in list, newest at bottom
  ...
  }, [selectedServerId]); // Remove 'paused' from dependency to avoid re-joining on pause toggle
    ...
    <div className="p-6 h-[calc(100vh-64px)] flex flex-col space-y-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#1f1f1f]/50 p-4 rounded-lg shadow-sm border border-[#2d3139]">
        ...
        <Space wrap>
          ...
          />
          <Space.Compact>
            <Button onClick={() => setFilter('Chat')} className={filter === 'Chat' ? 'text-blue-400 border-blue-400' : ''}>聊天</Button>
            <Button onClick={() => setFilter('Kill')} className={filter === 'Kill' ? 'text-red-400 border-red-400' : ''}>击杀</Button>
            <Button onClick={() => setFilter('Admin')} className={filter === 'Admin' ? 'text-purple-400 border-purple-400' : ''}>管理员</Button>
            <Button onClick={() => setFilter('')} icon={<ClearOutlined />}>重置</Button>
          </Space.Compact>
          <Input 
            placeholder="搜索内容..." 
            value={filter} 
      ...
      <div className="flex-1 bg-[#1e1e1e] rounded-lg p-4 overflow-hidden flex flex-col font-mono text-sm shadow-inner border border-gray-800">
        <div className="flex-1 overflow-y-auto custom-scrollbar" ref={scrollRef}>
          {filteredLogs.map((log, index) => (
            <div key={log.id || index} className="mb-1 hover:bg-[#2d2d2d] px-2 py-0.5 rounded transition-colors flex items-start">
              <span className="text-gray-500 mr-3 select-none whitespace-nowrap">
                [{new Date(log.timestamp).toLocaleTimeString()}]
              </span>
              <span className={`mr-3 font-bold uppercase w-16 inline-block text-center rounded px-1 text-xs whitespace-nowrap ${getLogColor(log.type)} bg-opacity-10 bg-current`}>
                {log.type}
              </span>
              <span className={`text-gray-300 break-words whitespace-pre-wrap ${log.type === 'chat' ? 'text-blue-100' : ''}`}>
                {log.content}
              </span>
            </div>
          ))}
