...
import { Table, Button, Modal, Form, Input, Select, message, Tag, Space, Typography, Card } from 'antd';
...
const CDKManager: React.FC = () => {
  const [cdks, setCdks] = useState<api.CDK[]>([]);
  ...
  const fetchCDKs = async () => {
    setLoading(true);
    try {
      const response = await api.getCDKs();
  ...
  const handleRedeem = async (values: any) => {
    try {
      const response = await api.redeemCDK(values);
